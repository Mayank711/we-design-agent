...
## v0.8 — Festival identity system + asset-delivery rules
- **Festival identity flow** (proven on Raksha Bandhan + Janmashtami, `docs/booktruck-festival-identity.md`):
  the five-slot system — ① underline · ② companion · ③ corner slot · ④ canvas · ⑤ narrative — the
  composition never changes, each festival only fills the slots. On-canvas flow card documents it for PMs.
- **`/screen-iterate`**: theme-concept rule (a concept must restyle every band; baked raster art only
  in matching-palette themes) + extract-don't-redraw rule (brand art is extracted per-node —
  lockups as SVG vectors, illustrations chroma-keyed — never hand-drawn).
- **`gotchas.md`**: inline `node.screenshot()` can render image fills blank (verify image-heavy
  nodes with server-side `get_screenshot`); "transparent" frame exports can bake the canvas grey
  #F5F5F5 (check corner alpha, flood-key if present).
- **Asset delivery rules**: app icons ship as **Android adaptive layers** (bg + fg sized to the 66dp
  safe zone + anydpi-v26 XML), never a padded square PNG alone; preview by compositing through the
  72dp mask. DPI ladders are generated by downscaling 1024px masters (`rescale()` in Figma — group
  resize does NOT scale children), never by upscaling.
- **Design rule**: festival symbols keep their real-world anatomy (a rakhi's medallion sits at the
  thread's centre, not its end) — preserve recognisable geometry, simplify only detailing.
- **Why:** the first shared-with-users round (festival logos + on-device install) surfaced delivery
  as a skill concern, not just design: what leaves Figma must survive launchers, masks, and DPI.
